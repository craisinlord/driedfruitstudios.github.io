...
# CurseForge download totals, approximate, captured 2026-09-12.
CF_DL = {
    "idas": 29_200_000, "integrated-api": 29_800_000, "integrated-villages": 10_900_000,
    "integrated-stronghold": 21_232_903, "integrated-cataclysm": 5_958_330,
    "integrated-simply-swords": 1_300_000, "integrated-cobblemon": 12_400,
    "antarchy": 74_674, "modern-morph": 1_500, "modern-apple-cows": 34_500,
    "sizeable-foliage": 84_000, "decorative-sniffer-eggs": 750,
    "cartographers-quills": 486, "dynamic-villager-trades-neoforge": 4_600,
    "dark-loading-screen-neoforge": 4_100, "electric-boogaloo": 845,
    "integrated-minecraft": 1_846_231,
}

# Overrides the Modrinth API summary with the live CurseForge description text.
SUMMARY_OVERRIDE = {
    "antarchy": "Antarchy is a massive dimension mod: 3 fleshed-out dimensions "
                "plus infinite randomly generated ones, 45+ mobs, 8 bosses, "
                "insane weapons, and dungeons.",
}
...
# Minecraft Marketplace listing data, once we have any: {slug: {"rating": 4.8,
# "ratingCount": 120, "price": 490}}. price is in Minecoins; 0 = free listing.
# Empty for now -- cards/detail pages just skip rating/price when absent.
MARKETPLACE = {}

def marketplace(slug):
    m = MARKETPLACE.get(slug, {})
    return m.get("rating"), m.get("ratingCount"), m.get("price")
...
for slug, category, status, featured in CURATED:
    ...
    if m:
        ...
        summary = SUMMARY_OVERRIDE.get(slug, (m.get("description") or "").strip())
    ...
    rating, ratingCount, price = marketplace(slug)
    entry = {
        "slug": slug,
        "name": name,
        "category": category,
...
for slug, name, summary in COMING_SOON_PACKS:
    products.append({
        ...
        "minecraftVersions": "", "downloads": 0,
        "rating": None, "ratingCount": None, "price": None,
